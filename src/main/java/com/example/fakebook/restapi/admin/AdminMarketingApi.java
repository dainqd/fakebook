...
public class AdminMarketingApi {
    ...
    public Page<MarketingDto> getList(@RequestParam(value = "page", required = false, defaultValue = "0") int page,
                                      ...
                                      @RequestParam(value = "status", required = false, defaultValue = "") Enums.MarketingStatus status) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("id").descending());
        if (status != null) {
            return marketingService.findAllByStatus(status, pageable).map(MarketingDto::new);
        }
        return marketingService.findAll(pageable).map(MarketingDto::new);
    }
